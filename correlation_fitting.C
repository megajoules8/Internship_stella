//correlation fitting, linear to get total histogram
#include "libPerso.h"
...
void correlation_fitting (){
  ...
  TH1F *h_qdc[6], *h_qdc_jj[4], *h_Energy_calib[3], *h_Energy_Ohmic_junction; //declaring TH1f variables [which is an array of objects of the class with a specified length]
  TH2F *h_qdc_qj;
  ...
  for (Long64_t i = 0; i < (Long64_t) f_in->GetEntries () - 10; i++)
    {
...

      {
        ...
        std::cout << qdc_coi.at (0) << " " << qdc_coi.at (1) << " "  << qdc_coi.at (2) << " " << qdc_coi.size  () << std::endl;
        //h_Energy_Ohmic_junction->Fill (qdc_coi.at (1)); //+ qdc_coi.at (0) + qdc_coi.at (2));
...
//TF1 *f_fit = new TF1 ("f_fit", myLin, 9700., 14000., 2);
    TF1 *corr_fit = new TF1 ("corr_fit", myLin, 5000., 25000., 2);
// display and fit
    corr_fit->SetNpx (1e3); 
    corr_fit->SetParameters (1000, 5/3 );

  h_qdc_qj->Fit (corr_fit, "RI");
  
  h_qdc_qj->DrawCopy ("SAME");

  //1.503 +/- 0.00802 output of slope for fitting
  //slope = rise/run = opposite over adjacent of angle
  //angle = tan^-1 of slope

/*Canvas *c_Energy_Ohmic_junction = new TCanvas ("c_Energy_Ohmic_junction", "c_Energy_Ohmic_junction", 700, 500);
  c_Energy_Ohmic_junction->Draw ();

  
      c_Energy_Ohmic_junction->cd ()->SetTickx ();
      c_Energy_Ohmic_junction->cd ()->SetTicky ();
      h_Energy_Ohmic_junction->GetXaxis ()->SetRangeUser (100., 8000.); //ohmic side =2 then divided by 2
      h_Energy_Ohmic_junction->Draw ();
 */
}
